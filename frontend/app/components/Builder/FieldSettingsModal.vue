...
<template>
	<Teleport to="body">
		<div v-if="open && field" class="modal-overlay" @click.self="close">
			<div class="modal">
				<div class="modal-header">
					<div class="modal-title">
						<UISysIcon v-if="fieldMeta" :icon="fieldMeta.icon" class="title-icon" />
						<h2>{{ $t(`fields.${field.type}.label`) }} {{ $t("builder.fieldSettings.edit") }}</h2>
					</div>
					...
					<div v-if="hasOptions" class="form-group">
						...
						<div class="options-list">
							<div v-for="(option, index) in field.options || []" :key="index" class="option-row">
								<UISysIcon icon="fa-solid fa-grip-vertical" class="option-drag" />
								<input
									:value="option"
									class="input"
									type="text"
